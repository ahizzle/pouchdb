...
function WebSqlPouch(opts, callback) {
  ...
  function runMigration5(tx, callback) {
    ...
    function migrateAttachments(tx) {
      // need to actually populate the table. we can rely on the old
      // system of storing the refs in a json object in the attach-store
      var offset = 0;
      var pageSize = 10;
      function nextPage() {
        ...
        function migrateAttachment(digest, refs) {
          refs.forEach(function (ref) {
            var idx = utils.lastIndexOf(ref, '@');
            var docId = ref.substring(0, idx);
            var rev = ref.substring(idx + 1);
            ...
            tx.executeSql(sql, [docId, rev], function (tx, res) {
              ...
              var sql = 'INSERT INTO ' + ATTACH_AND_SEQ_STORE +
                ' (digest, seq) VALUES (?, ?)';
              tx.executeSql(sql, [digest, seq], checkDone);
            });
          });
        }
        tx.executeSql(sql, [], function (tx, res) {
          ...
          for (var i = 0; i < res.rows.length; i++) {
            ...
            var refs = Object.keys(json.refs || {});
            migrateAttachment(digest, refs);
          }
        });
      }
      nextPage();
    ...
    var attachAndRev = 'CREATE TABLE IF NOT EXISTS ' + ATTACH_AND_SEQ_STORE +
      ' (digest, seq)';
    tx.executeSql(attachAndRev, [], function (tx) {
      tx.executeSql(
          ATTACH_AND_SEQ_STORE_SEQ_INDEX_SQL, [], function (tx) {
        tx.executeSql(
